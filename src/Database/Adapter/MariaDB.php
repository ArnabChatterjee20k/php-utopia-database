...
class MariaDB extends SQL
{
    ...
    /**
     * Process spatial value from MariaDB - convert from binary format to array
     *
     * @param mixed $value
     * @return mixed
     */
    protected function processSpatialValue(mixed $value): mixed
    {
        ...
        // MariaDB returns spatial data in binary format when using SELECT *
        // We need to convert it to WKT first using ST_AsText
        if (is_string($value)) {
            // If it's already a WKT string (from ST_AsText), convert to array
            ...
            // If it's binary data (raw bytes), we need to convert using SQL
            // For now, return the raw value and let the higher level handle it
            return $value;
        }
    ...
    /**
     * Create Documents with spatial data support
     *
     * @param string $collection
    ...
    public function createDocuments(string $collection, array $documents): array
    {
        ...
        try {
            ...
            // Return the documents with their IDs and sequences
            return $documents;
     ...
     * @return bool
     */
    private function isWKTString(string $value): bool
    {
        $value = trim($value);
        return preg_match('/^(POINT|LINESTRING|POLYGON|MULTIPOINT|MULTILINESTRING|MULTIPOLYGON|GEOMETRYCOLLECTION)\s*\(/i', $value);
    }
}
